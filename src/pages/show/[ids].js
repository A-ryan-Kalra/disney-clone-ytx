    ...
    <div>
      ...
      {!session ? (
        ...
        <section className="relative z-50 overflow-hidden">
          ...
          <div
            className="absolute bottom-16 md:inset-y-auto bg-gray-600/40 rounded-lg    p-2 md:w-[1100px] md:bottom-10 inset-x-4
            md:inset-x-12 right-10 space-y-6 z-50 h-max"
          >
            ...
            <div className="flex items-center space-x-3 md:space-x-5">
              ...
              <button
                className="text-sm text-black md:text-base flex items-center justify-center bg-black/30 py-2 px-6 rounded hover:bg-[#c6c6c6]"
            ...
            <p className="text-sm md:text-base">
              ...
              {result.runtime && Math.floor(result.runtime / 60)}
              {result.runtime && "h"}
              {result.runtime && Math.floor(result.runtime % 60)}
              {result.runtime && "m •"}
          ...
          {showPlayer && (
            ...
            w-full z-50"
              onClick={() => console.log(setShowPlayer(false))}
            />
          )}
          <div
          ...
          >
            ...
            <div className="relative text-white pt-[100%] md:pt-[50.25%]">
              <ReactPlayer
...
export async function getServerSideProps(context) {
  ...
  const request = await fetch(
    `https://api.themoviedb.org/3/tv/${ids}?api_key=${process.env.API_KEY}&append_to_response=videos`
